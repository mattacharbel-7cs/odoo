...
class WifiCustomerUsernameController(http.Controller):
    ...
    def _find_partner(self, values):
        ...
        if username:
            return self._search_single(Partner, [('wifi_username', '=', username)])
    ...
    def update_username(self, **kwargs):
        ...
        try:
            partner.write({'wifi_username': username})
        except ValidationError:
            return self._error('username_already_exists')
        except Exception:
            _logger.exception("Failed to update WiFi username for partner %s", partner.id)
